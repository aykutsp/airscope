...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let out_path = std::env::args().nth(1).unwrap_or_else(|| "samples/demo-01.pcap".to_string());

    let mut w =
        PcapFileWriter::create(&out_path, LINKTYPE_IEEE802_11, 4096).map_err(|e| e.to_string())?;
    ...
    for round in 0..3 {
        for (bssid_s, ssid) in aps {
            ...
            let frame =
                build_beacon(bssid, effective_ssid, 6, 100, round == 2 && *ssid == "home-network");
    ...
    w.write(&CapturedPacket { timestamp_micros: ts, data: deauth }).map_err(|e| e.to_string())?;
